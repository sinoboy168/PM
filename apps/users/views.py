...
class IndexView(View):
    def get(self, request):
        ...
        context = {
            ...
            'tr_list': tr_list,
            'dep_list': dep_list,
            'Get_Latest_Trouble':{'name':'A','get_handle_result_display':'GOOD','event_time':datetime.datetime.now()}
        }
...
# 上传修改用户头像
######################################
class ChangeUserAvatarUploadView(View):
    def post(self, request):
...
# 选择修改用户头像
######################################
class ChangeUserAvatarChoseView(View):
    def post(self, request):
        user = request.user
...
# 发送修改用户邮箱验证码
######################################
class SendChangeUserEmailCodeView(View):
    def post(self, request):
        email = request.POST.get('email')
        if UserProfile.objects.filter(email=email):
...
# 用户登录信息
######################################
class UserLoginRecordView(View):
    def get(self, request):
        # 页面选择
...
# 用户操作信息
######################################
class UserOperationRecordView(View):
    def get(self, request):
        # 页面选择
        web_chose_left_1 = 'log_management'
